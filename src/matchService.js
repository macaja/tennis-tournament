...
function updatePointScore(player, gameScore) {
    if (gameScore.type === 'Deuce') {
        ...
        if (player === 'PlayerA') {
            if (scoreA === 'Advantage')
                return { type: 'Game', scoreA: 'Love', scoreB: 'Love' };
            if (scoreA === 'Thirty' && scoreB === 'Forty')
                return { type: 'Deuce' };
...
function isSetWon(player, setScore) {
    if (setScore.type === 'Set') {
        const { gamesA, gamesB } = setScore;
        return ((player === 'PlayerA' && (gamesA === 6 && gamesB <= 4 || gamesA === 7)) ||
...
function isMatchWon(winner, currentSet, previousSets) {
    const setsWon = previousSets.reduce((acc, set) => {
        if (isSetWon('PlayerA', set))
            acc.playerA++;
        if (isSetWon('PlayerB', set))
            acc.playerB++;
        return acc;
    }, { playerA: 0, playerB: 0 });
...
function playPoint(winner, matchState) {
